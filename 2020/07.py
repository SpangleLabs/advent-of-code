...
rules = """muted lime bags contain 1 wavy lime bag, 1 vibrant green bag, 3 light yellow bags.
...
# Map of contained bag colour to possible container bag colours
bag_map = defaultdict(lambda : [])
for rule in rules.split("\n"):
    container, contains = rule.split(" contain ")
    container_col = container[:-len(" bags")]
    contained_cols = []
    ...
    for contained_col in contained_cols:
        bag_map[contained_col].append(container_col)

target = "shiny gold"
possible_bags = set(bag_map[target])
new_bags = set(bag_map[target])
while new_bags:
    old_bags = new_bags
    new_bags = set()
    for new_bag in old_bags:
        for bag in bag_map[new_bag]:
