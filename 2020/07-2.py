...
# Map of container bag colour to tuple of count and contained bag colour
...
for rule in rules.split("\n"):
    ...
    if contains != "no other bags.":
        contained_cols = [
            (int(bag.split(" ")[0]), " ".join(bag.split(" ")[1:-1]))
            for bag
            in contains.split(", ")
        ]
    bag_map[container_col] = contained_cols

target = "shiny gold"
total_bags = 0
bags_to_add = bag_map[target]
while bags_to_add:
    checking = bags_to_add[:]
    bags_to_add = []
    for bag in checking:
        total_bags += bag[0]
        bags_to_add += [(n[0] * bag[0], n[1]) for n in bag_map[bag[1]]]
print(total_bags)
