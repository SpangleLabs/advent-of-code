...
required_fields = set(field_validation.keys())
field_validation["cid"] = lambda x: True
for passport in passports.split("\n\n"):
    ...
    for field in fields:
        ...
        if key not in field_validation:
            print(f"Invalid field: {key}")
            valid = False
            break
        if not field_validation[key](value):
            print(f"Field {key} has invalid value {value}")
            valid = False
            break
        known_fields.add(key)
    if required_fields - known_fields:
        print(f"Missing fields: {required_fields - known_fields}")
        valid = False
    if valid:
        valid_count += 1
print(valid_count)
