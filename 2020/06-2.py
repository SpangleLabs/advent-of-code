...
total_yes = 0
for group in group_answers.split("\n\n"):
    group_questions = None
    for person in group.split("\n"):
        if group_questions is None:
            group_questions = set(person)
            continue
        group_questions = group_questions.intersection(set(person))
    total_yes += len(group_questions)
print(total_yes)
