...
class Instruction:
    ...
    def parse_input(cls, line: str) -> "Instruction":
        ...
        return cls(
            ...
            Coords3D(max(x_vals) + 1, max(y_vals) + 1, max(z_vals) + 1),
            state == "on"
        )
...
class Reactor:
    ...
    def __init__(self, lookup: "LookupTables"):
        self.lookup = lookup
        self.map = Map3D(len(lookup.x_lookup), len(lookup.y_lookup), len(lookup.z_lookup), False)
        # self.cuboids = set()

    def apply_instruction(self, instruction: Instruction) -> None:
        mapped = self.lookup.lookup(instruction.cuboid)
        for x in range(mapped.start.x, mapped.end.x):
            for y in range(mapped.start.y, mapped.end.y):
                for z in range(mapped.start.z, mapped.end.z):
                    self.map.set_value(Coords3D(x, y, z), instruction.state)

    def count_on(self) -> int:
        return sum(
            self.lookup.coords_size(coord)
            for coord in self.map.all_coords_with_value(True)
        )
...
class LookupTables:
    def __init__(
        ...
        self.x_reverse = {}
        self.x_sizes = {}
        x_sorted = list(sorted(set(x_values)))
        for i, x in enumerate(x_sorted):
            ...
            if i + 1 == len(x_sorted):
                ...
                self.x_sizes[i] = x_sorted[i + 1] - x
        self.y_lookup = {}
        self.y_reverse = {}
        self.y_sizes = {}
        y_sorted = list(sorted(set(y_values)))
        for i, y in enumerate(y_sorted):
            ...
            if i + 1 == len(y_sorted):
                ...
                self.y_sizes[i] = y_sorted[i + 1] - y
        self.z_lookup = {}
        self.z_reverse = {}
        self.z_sizes = {}
        z_sorted = list(sorted(set(z_values)))
        for i, z in enumerate(z_sorted):
            ...
            if i + 1 == len(z_sorted):
                ...
                self.z_sizes[i] = z_sorted[i + 1] - z

    def lookup_coords(self, coord: Coords3D) -> Coords3D:
        return Coords3D(
            self.x_lookup[coord.x],
            self.y_lookup[coord.y],
            self.z_lookup[coord.z]
        )

    def lookup(self, cuboid: Cuboid) -> Cuboid:
        return Cuboid(
            self.lookup_coords(cuboid.start),
            self.lookup_coords(cuboid.end)
        )

    def reverse_coords(self, coord: Coords3D) -> Coords3D:
        return Coords3D(
            self.x_reverse[coord.x],
            self.y_reverse[coord.y],
            self.z_reverse[coord.z]
        )

    def reverse(self, cuboid: Cuboid) -> Cuboid:
        return Cuboid(
            self.reverse_coords(cuboid.start),
            self.reverse_coords(cuboid.end)
        )

    def coords_size(self, coords: Coords3D) -> int:
        return (
            (self.x_sizes[coords.x])
            * (self.y_sizes[coords.y])
            * (self.z_sizes[coords.z])
        )


def build_lookup_tables(cuboids: List[Cuboid]) -> LookupTables:
    return LookupTables(
        sum(([c.start.x, c.end.x] for c in cuboids), start=[]),
        sum(([c.start.y, c.end.y] for c in cuboids), start=[]),
        sum(([c.start.z, c.end.z] for c in cuboids), start=[])
    )


def _main() -> str:
    my_input = load_lines()
    instructions = [Instruction.parse_input(line) for line in my_input]
    lookup = build_lookup_tables([i.cuboid for i in instructions])
    print("Built lookups")
    reactor = Reactor(lookup)
    print("Built reactor map")
