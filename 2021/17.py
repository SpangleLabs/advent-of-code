...
def position_after_step(start_velocity: Coords2D, step: int) -> Coords2D:
    ...
    y_val = (start_velocity.y * step) - triangle_number(step)
    ...
    for try_y in range(min_y, max_y + 1):
        for try_x in range(min_x, max_x + 1):
            ...
            while not out_of_bounds and not landed_in_target:
                ...
                step += 1
            max_height = triangle_number(try_vel.y) if try_y > 0 else 0
            if landed_in_target and max_height > max_max_height:
                max_max_height = max_height
                max_vel = try_vel
                print(f"New highest point: {max_max_height}, from velocity {max_vel}")
    return str(max_max_height)
